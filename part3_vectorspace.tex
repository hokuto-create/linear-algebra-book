...
\begin{goal}
\begin{itemize}
\item 線形写像の定義(和とスカラー倍を保つ写像)を理解し、
与えられた写像が線形かどうかを判定・証明できるようになる。
また、線形写像が\textbf{基底の行き先だけで決まる}ことを使えるようになる。
\item \textbf{核}と\textbf{像}を計算し、核による単射性の判定を
使えるようになる。
\item \textbf{次元定理} $\dim V = \dim \mathrm{Ker}\,f + \dim \mathrm{Im}\,f$
の証明を理解し、微分・積分・数列など様々な線形写像に適用できるようになる。
\end{itemize}
\end{goal}

\section{動機 ― 空間の次は、空間のあいだの写像}

第I部で、行列 $A$ を掛ける操作 $\vect{x} \mapsto A\vect{x}$ が
$\R^n$ から $\R^m$ への「変換」であることを学び、
それが和とスカラー倍を保つこと(定理 \ref{thm:matvec-linear})も確かめた。
いま舞台はベクトル空間一般に広がった。ならば写像の概念も広げよう
――行列という道具立てを離れ、\textbf{「和とスカラー倍を保つ」という
性質だけ}を要求するのである。

こうして得られる線形写像の世界には、思いがけない住人が加わる。
なんと\textbf{微分も積分も線形写像}である。高校以来使ってきた
「項別に微分してよい」という規則は、線形性の主張にほかならない。
ベクトル空間の公理化(第 \ref{chap:vs} 章)が多項式や関数を
ベクトルの仲間にしたように、線形写像の公理化は微分や積分を
「行列の仲間」にする。そして第II部で調べた解空間や階数の理論が、
\textbf{核}と\textbf{像}という言葉で一斉に一般化される。
本章の頂点は、自由度定理の抽象版である\textbf{次元定理}の証明である。
...
出会った概念の、抽象版である。まず、定義から直ちに従う基本性質を
まとめておく。

\begin{teiri}[線形写像の基本性質]\label{thm:linmap-basic}
線形写像 $f : V \to W$ について、次が成り立つ。
\begin{enumerate}
\item $f(\vect{0}) = \vect{0}$(零ベクトルは零ベクトルにうつる)。
\item $f(-\vect{v}) = -f(\vect{v})$。
\item 線形結合は線形結合にうつる:
\[
f(c_1\vect{v}_1 + \cdots + c_k\vect{v}_k)
= c_1 f(\vect{v}_1) + \cdots + c_k f(\vect{v}_k).
\]
\end{enumerate}
\end{teiri}

\noindent\textbf{(証明)}\quad
(1) $f(\vect{0}) = f(0 \cdot \vect{0}) = 0 \cdot f(\vect{0}) = \vect{0}$
(スカラー倍の保存と、$0\vect{w} = \vect{0}$(定理 \ref{thm:vs-basic}))。
(2) $f(-\vect{v}) = f((-1)\vect{v}) = (-1)f(\vect{v}) = -f(\vect{v})$。
(3) 和の保存を繰り返し使って項ごとに分け、各項でスカラー倍の保存を使う。
$\blacksquare$

(1) は実用上重要である:\textbf{$f(\vect{0}) \neq \vect{0}$ なら、
その時点で線形写像ではない}(反例 \ref{hanrei:linmap-affine})。

\begin{rei}[行列の定める写像]
$m \times n$ 行列 $A$ に対し、$f(\vect{x}) = A\vect{x}$ は
$\R^n \to \R^m$ の線形写像である(定理 \ref{thm:matvec-linear})。
回転・鏡映・拡大などの幾何的変換(第I部)はすべてこの形だった。
逆に、$\R^n \to \R^m$ の線形写像はすべて行列で書けること
(定理 \ref{thm:lin-is-matrix})も第I部で見た。
\end{rei}
...
\begin{rei}[評価写像]
点を一つ決めて多項式の値を読む写像、たとえば
$T : \R[x]_2 \to \R$, $T(p) = p(1)$ も線形写像である:
$(p + q)(1) = p(1) + q(1)$, $(cp)(1) = c\,p(1)$。
「$x = 1$ を代入する」という操作さえ、線形写像なのである。
\end{rei}

\section{線形写像は基底の行き先で決まる}

線形写像の著しい特徴は、\textbf{ごく少数の情報で全体が決まる}ことである。

\begin{teiri}[基底の行き先が写像を決める]\label{thm:linmap-basis}
$V$ を有限次元ベクトル空間、$\vect{e}_1, \dots, \vect{e}_n$ をその基底とし、
$W$ のベクトル $\vect{w}_1, \dots, \vect{w}_n$ を任意に指定する。
このとき、
\[
f(\vect{e}_i) = \vect{w}_i \qquad (i = 1, \dots, n)
\]
を満たす線形写像 $f : V \to W$ が\textbf{ただ一つ}存在する。
\end{teiri}

\noindent\textbf{(証明)}\quad
存在:任意の $\vect{v} \in V$ は座標を使って
$\vect{v} = c_1\vect{e}_1 + \cdots + c_n\vect{e}_n$ とただ一通りに書ける
(定理 \ref{thm:coord-unique})から、
\[
f(\vect{v}) = c_1\vect{w}_1 + \cdots + c_n\vect{w}_n
\]
と定めれば $f$ は矛盾なく定義され、$f(\vect{e}_i) = \vect{w}_i$ を満たす。
線形性を確かめる。$\vect{v}, \vect{v}'$ の座標を $(c_i)$, $(c'_i)$ とすると、
$\vect{v} + \vect{v}' = \sum_i (c_i + c'_i)\vect{e}_i$ だから
座標の一意性により $\vect{v} + \vect{v}'$ の座標は $(c_i + c'_i)$ であり、
\[
f(\vect{v} + \vect{v}') = \sum_i (c_i + c'_i)\vect{w}_i
= \sum_i c_i\vect{w}_i + \sum_i c'_i\vect{w}_i = f(\vect{v}) + f(\vect{v}').
\]
スカラー倍も同様である。

一意性:$g$ も条件を満たす線形写像とすると、任意の $\vect{v}$ に対し
定理 \ref{thm:linmap-basic}(3) より
\[
g(\vect{v}) = g\Bigl(\sum_i c_i\vect{e}_i\Bigr) = \sum_i c_i\,g(\vect{e}_i)
= \sum_i c_i\vect{w}_i = f(\vect{v}).
\]
よって $g = f$ である。 $\blacksquare$

$n$ 次元空間からの線形写像は、基底 $n$ 本の行き先という
\textbf{有限個のデータで完全に決まる}。「線形写像を行列で表す」
(次章の表現行列)ことができるのは、この定理のおかげである。
...
\begin{teiri}\label{thm:kerim-sub}
$\mathrm{Ker}\,f$ は $V$ の部分空間、$\mathrm{Im}\,f$ は $W$ の部分空間である。
\end{teiri}

\noindent\textbf{(証明)}\quad
核:$f(\vect{0}) = \vect{0}$ より $\vect{0} \in \mathrm{Ker}\,f$。
$\vect{u}, \vect{v} \in \mathrm{Ker}\,f$ ならば
$f(\vect{u} + \vect{v}) = f(\vect{u}) + f(\vect{v}) = \vect{0} + \vect{0}
= \vect{0}$、$f(c\vect{u}) = c\,f(\vect{u}) = \vect{0}$ だから、
和もスカラー倍も核に留まる。

像:$\vect{0} = f(\vect{0}) \in \mathrm{Im}\,f$。
$f(\vect{u}), f(\vect{v}) \in \mathrm{Im}\,f$ の和は
$f(\vect{u}) + f(\vect{v}) = f(\vect{u} + \vect{v}) \in \mathrm{Im}\,f$、
スカラー倍は $c\,f(\vect{u}) = f(c\vect{u}) \in \mathrm{Im}\,f$ である。
$\blacksquare$
...
\begin{teiri}[単射性の判定]\label{thm:linmap-inj}
線形写像 $f$ が単射(一対一)であることと、
$\mathrm{Ker}\,f = \{\vect{0}\}$ は同値である。
\end{teiri}

\noindent\textbf{(証明)}\quad
$f$ が単射なら、$f(\vect{v}) = \vect{0} = f(\vect{0})$ を満たす
$\vect{v}$ は $\vect{0}$ しかないから $\mathrm{Ker}\,f = \{\vect{0}\}$。
逆に $\mathrm{Ker}\,f = \{\vect{0}\}$ とする。$f(\vect{u}) = f(\vect{v})$
ならば、線形性より
$f(\vect{u} - \vect{v}) = f(\vect{u}) - f(\vect{v}) = \vect{0}$、
すなわち $\vect{u} - \vect{v} \in \mathrm{Ker}\,f = \{\vect{0}\}$ だから
$\vect{u} = \vect{v}$。よって単射である。 $\blacksquare$

「写像がつぶれていないこと」を、たった一点 $\vect{0}$ の逆像だけで
判定できる――線形性のご利益である。

\begin{teiri}[次元定理]\label{thm:dimension}
...
\noindent\textbf{(証明)}\quad
$\dim V = n$、$\dim \mathrm{Ker}\,f = k$ とおき、
$\mathrm{Ker}\,f$ の基底 $\vect{u}_1, \dots, \vect{u}_k$ をとる。
拡張定理 \ref{thm:dim-extend}(2) により、これに $r = n - k$ 本を
付け足して $V$ の基底
\[
\vect{u}_1, \dots, \vect{u}_k,\ \vect{v}_1, \dots, \vect{v}_r
\]
が作れる。このとき
\textbf{$f(\vect{v}_1), \dots, f(\vect{v}_r)$ が $\mathrm{Im}\,f$ の基底}
であることを示せば、$\dim \mathrm{Im}\,f = r = n - k$ となって証明が終わる。

生成:$\mathrm{Im}\,f$ の任意の元は $f(\vect{v})$ の形である。
$\vect{v} = a_1\vect{u}_1 + \cdots + a_k\vect{u}_k
+ b_1\vect{v}_1 + \cdots + b_r\vect{v}_r$ と基底で書いて $f$ を施すと、
$f(\vect{u}_i) = \vect{0}$ だから
\[
f(\vect{v}) = b_1 f(\vect{v}_1) + \cdots + b_r f(\vect{v}_r).
\]
よって $f(\vect{v}_1), \dots, f(\vect{v}_r)$ は $\mathrm{Im}\,f$ を生成する。

独立:$c_1 f(\vect{v}_1) + \cdots + c_r f(\vect{v}_r) = \vect{0}$ とすると、
線形性より $f(c_1\vect{v}_1 + \cdots + c_r\vect{v}_r) = \vect{0}$、
すなわち $c_1\vect{v}_1 + \cdots + c_r\vect{v}_r \in \mathrm{Ker}\,f$。
核の元は核の基底で書けるから、ある $d_1, \dots, d_k$ により
\[
c_1\vect{v}_1 + \cdots + c_r\vect{v}_r
= d_1\vect{u}_1 + \cdots + d_k\vect{u}_k,
\]
移項すれば
$d_1\vect{u}_1 + \cdots + d_k\vect{u}_k - c_1\vect{v}_1 - \cdots
- c_r\vect{v}_r = \vect{0}$。
これは $V$ の基底の線形結合だから、係数はすべて $0$、
とくに $c_1 = \cdots = c_r = 0$ である。 $\blacksquare$
...
\begin{teiri}[合成写像の線形性]\label{thm:linmap-comp}
$f : V \to W$、$g : W \to U$ が線形写像ならば、合成
$g \circ f : V \to U$($\vect{v} \mapsto g(f(\vect{v}))$)も線形写像である。
\end{teiri}

\noindent\textbf{(証明)}\quad
$(g \circ f)(\vect{u} + \vect{v}) = g(f(\vect{u}) + f(\vect{v}))
= g(f(\vect{u})) + g(f(\vect{v}))$、スカラー倍も同様。 $\blacksquare$

行列の定める写像でいえば、合成は行列の積に対応する(第 \ref{chap:matmul} 章)。
この対応が一般の線形写像でも成り立つことは、次章で表現行列を導入すると
はっきり見える。
...
\noindent\textbf{(証明)}\quad
座標写像を $\varphi$ とする。定理 \ref{thm:linmap-basis} の証明で見たとおり、
和の座標は座標の和、スカラー倍の座標は座標のスカラー倍だから
$\varphi$ は線形である。$\varphi(\vect{v}) = \vect{0}$ は
$\vect{v} = 0\vect{e}_1 + \cdots + 0\vect{e}_n = \vect{0}$ を意味するから
$\mathrm{Ker}\,\varphi = \{\vect{0}\}$ で単射
(定理 \ref{thm:linmap-inj})。また任意の $(c_1, \dots, c_n) \in \R^n$ は
$c_1\vect{e}_1 + \cdots + c_n\vect{e}_n$ の座標だから全射。
よって $\varphi$ は同型写像である。 $\blacksquare$
...
\begin{hanrei}[一次関数は線形写像とは限らない]\label{hanrei:linmap-affine}
高校で「一次関数」と呼んだ $f(x) = 2x + 3$($\R \to \R$)は、
線形写像では\textbf{ない}。実際 $f(0) = 3 \neq 0$ であり、
定理 \ref{thm:linmap-basic}(1) に反する
(和も保たない:$f(1 + 1) = 7$ だが $f(1) + f(1) = 10$)。
線形写像なのは $b = 0$ の場合 $f(x) = ax$ だけである。
一般に、平行移動 $\vect{v} \mapsto \vect{v} + \vect{b}$
($\vect{b} \neq \vect{0}$)は線形でない。
線形写像+平行移動の形の写像は\textbf{アフィン写像}と呼ばれ、
部分空間とアフィン部分空間(第 \ref{chap:subspace} 章の深掘り)と
同じ関係にある。
\end{hanrei}

\begin{hanrei}[$f(\vect{0}) = \vect{0}$ でも線形とは限らない]\label{hanrei:linmap-square}
$f : \R \to \R$, $f(x) = x^2$ は $f(0) = 0$ を満たすが線形でない。
スカラー倍が保たれない:$f(2 \cdot 1) = 4$ だが $2f(1) = 2$。
同様に $g : \R^2 \to \R$, $g(x, y) = xy$ も $g(0,0) = 0$ だが、
$g(2, 2) = 4 \neq 2\,g(1, 1) = 2$ で線形でない。
$f(\vect{0}) = \vect{0}$ は線形性の\textbf{必要条件にすぎず}、
判定には和とスカラー倍の保存を正面から確かめる必要がある。
\end{hanrei}

\section{例題}

線形性の判定、核と像の計算、次元定理の運用を練習する。
以下は\textbf{解答つきの例題}である。手を動かして追ってほしい。

\begin{rei}[例題・基本(線形性の判定)]\label{rei:linmap-check}
次の写像 $\R^2 \to \R^2$ は線形写像か。
\[
f(x, y) = (2x + y,\ x - 3y), \qquad g(x, y) = (x + 1,\ y)
\]
\textbf{解.}\ $f$ は線形である。
$\vect{u} = (x_1, y_1)$, $\vect{v} = (x_2, y_2)$ に対し
\[
f(\vect{u} + \vect{v})
= \bigl(2(x_1 + x_2) + (y_1 + y_2),\ (x_1 + x_2) - 3(y_1 + y_2)\bigr)
= f(\vect{u}) + f(\vect{v}),
\]
$f(c\vect{u}) = (2cx_1 + cy_1,\ cx_1 - 3cy_1) = c\,f(\vect{u})$。
実は $f(\vect{x}) = \mat{2 & 1 \\ 1 & -3}\vect{x}$ と行列で書ける。

$g$ は線形でない。$g(0, 0) = (1, 0) \neq (0, 0)$ だから、
定理 \ref{thm:linmap-basic}(1) に反する。
\end{rei}

\begin{rei}[例題・基本(核と像の計算)]\label{rei:linmap-kerim}
$A = \mat{1 & 2 \\ 2 & 4}$ の定める線形写像
$f : \R^2 \to \R^2$ の核と像を求め、次元定理を確かめよ。\\
\textbf{解.}\ 核:$A\vect{x} = \vect{0}$ すなわち $x + 2y = 0$
($2$ 行目は $1$ 行目の $2$ 倍)を解いて
$\vect{x} = t\mat{-2 \\ 1}$。よって
$\mathrm{Ker}\,f = \left\langle \mat{-2 \\ 1} \right\rangle$、次元は $1$。

像:列ベクトルの張る空間で、$\mat{2 \\ 4} = 2\mat{1 \\ 2}$ だから
$\mathrm{Im}\,f = \left\langle \mat{1 \\ 2} \right\rangle$、次元は $1$
($\mathrm{rank}\,A = 1$)。

次元定理:$\dim \R^2 = 2 = 1 + 1$ で成立している。
「入力 $2$ 次元のうち、$1$ 次元分がつぶれ、$1$ 次元分が生き残る」。
\end{rei}

\begin{rei}[例題・基本(基底の行き先から写像を求める)]\label{rei:linmap-fromdata}
線形写像 $f : \R^2 \to \R^2$ が
\[
f\mat{1 \\ 0} = \mat{1 \\ 3}, \qquad f\mat{0 \\ 1} = \mat{2 \\ -1}
\]
を満たすとき、$f\mat{4 \\ 5}$ と、一般の $f\mat{x \\ y}$ を求めよ。\\
\textbf{解.}\ $\mat{4 \\ 5} = 4\mat{1 \\ 0} + 5\mat{0 \\ 1}$ だから、
線形性(定理 \ref{thm:linmap-basic}(3))より
\[
f\mat{4 \\ 5} = 4\mat{1 \\ 3} + 5\mat{2 \\ -1} = \mat{14 \\ 7}.
\]
一般には
$f\mat{x \\ y} = x\mat{1 \\ 3} + y\mat{2 \\ -1} = \mat{x + 2y \\ 3x - y}$、
行列で書けば $f(\vect{x}) = \mat{1 & 2 \\ 3 & -1}\vect{x}$
(基底の行き先が列に並ぶ)。基底 $2$ 本の行き先だけで、
平面全体の写像が完全に決まった(定理 \ref{thm:linmap-basis})。
\end{rei}

\begin{rei}[例題・標準(評価写像の核と像)]\label{rei:linmap-eval}
$T : \R[x]_2 \to \R$, $T(p) = p(1)$ について、
$\mathrm{Ker}\,T$ の次元と基底を求め、次元定理を確かめよ。\\
\textbf{解.}\ $T(1) = 1 \neq 0$ より $\mathrm{Im}\,T$ は $\{0\}$ ではなく、
$\R$ の部分空間だから $\mathrm{Im}\,T = \R$(次元 $1$。
定理 \ref{thm:dim-sub}(2) を $V = \R$ に使った)。
次元定理より
\[
\dim \mathrm{Ker}\,T = \dim \R[x]_2 - \dim \mathrm{Im}\,T = 3 - 1 = 2.
\]
基底を具体的に求める。$p(1) = 0$ を満たす多項式として
$x - 1$ と $x^2 - 1$ が取れる。この $2$ 本は独立
(次数が違うので一方が他方の定数倍にならない)で、
$\dim \mathrm{Ker}\,T = 2$ の空間の独立な $2$ 本だから基底である
(定理 \ref{thm:dim-ncheck})。
因数定理の言葉でいえば、$\mathrm{Ker}\,T$ は「$x - 1$ で割り切れる
$2$ 次以下の多項式」の全体である。
\end{rei}

\begin{rei}[例題・標準($\R^3$ からの写像)]\label{rei:linmap-r3}
$A = \mat{1 & 0 & 1 \\ 0 & 1 & 1 \\ 1 & 1 & 2}$ の定める線形写像
$f : \R^3 \to \R^3$ の核と像の次元を求め、次元定理を確かめよ。\\
\textbf{解.}\ 核:$A\vect{x} = \vect{0}$ は
$x + z = 0$, $y + z = 0$, $x + y + 2z = 0$ で、第 $3$ 式は
第 $1$ 式+第 $2$ 式なので落とせる。$z = t$ とおくと
$\vect{x} = t\mat{-1 \\ -1 \\ 1}$、
よって $\mathrm{Ker}\,f = \left\langle \mat{-1 \\ -1 \\ 1} \right\rangle$
で次元 $1$。

像:列ベクトル $3$ 本の張る空間である。第 $3$ 列 = 第 $1$ 列 + 第 $2$ 列
だから、独立な第 $1$・第 $2$ 列が基底で、次元は $2$
($\mathrm{rank}\,A = 2$。問 \ref{q:sub-ex9} で見た平面 $x + y - z = 0$
である)。

次元定理:$3 = 1 + 2$ で成立している。
\end{rei}

\begin{rei}[例題・標準(単射・全射・同型の判定)]\label{rei:linmap-iso}
$T : \R[x]_2 \to \R^3$, $T(p) = \bigl(p(0),\ p(1),\ p(2)\bigr)$ が
同型写像であることを示せ。\\
\textbf{解.}\ 線形性:$(p + q)(a) = p(a) + q(a)$,
$(cp)(a) = c\,p(a)$ が各点 $a = 0, 1, 2$ で成り立つから、
$T$ は線形である。

単射:$T(p) = \vect{0}$ とすると $p(0) = p(1) = p(2) = 0$、
すなわち $2$ 次以下の多項式 $p$ が相異なる $3$ 点で $0$ になる。
$p$ が零多項式でなければ、因数定理により $p$ は
$x(x - 1)(x - 2)$ で割り切れて次数が $3$ 以上になり矛盾。
よって $p = 0$、$\mathrm{Ker}\,T = \{0\}$ で単射である
(定理 \ref{thm:linmap-inj})。

全射:次元定理より
$\dim \mathrm{Im}\,T = \dim \R[x]_2 - \dim \mathrm{Ker}\,T = 3 - 0 = 3$。
$\mathrm{Im}\,T$ は $\R^3$ の $3$ 次元部分空間だから
$\mathrm{Im}\,T = \R^3$(定理 \ref{thm:dim-sub}(2))で全射である。

よって $T$ は同型写像である。「$2$ 次以下の多項式は、相異なる $3$ 点での
値を指定すればただ一つに決まる」――第I部の多項式フィッティング
(例 \ref{rei:gauss-fit})の背後にあった事実の、写像による言い換えである。
\end{rei}

\begin{matome}
\begin{itemize}
\item \textbf{線形写像}=和とスカラー倍を保つ写像。
$f(\vect{0}) = \vect{0}$ は必要条件(反例 \ref{hanrei:linmap-affine})だが
十分条件ではない(反例 \ref{hanrei:linmap-square})。
行列写像だけでなく、\textbf{微分・積分・シフト・評価}も線形写像である。
\item 線形写像は\textbf{基底の行き先だけで完全に決まる}
(定理 \ref{thm:linmap-basis})。これが次章の表現行列の根拠になる。
\item \textbf{核} $\mathrm{Ker}\,f$(つぶれて $\vect{0}$ になる入力全体)と
\textbf{像} $\mathrm{Im}\,f$(出力全体)は部分空間であり、
$f$ が単射 $\iff \mathrm{Ker}\,f = \{\vect{0}\}$。
\item \textbf{次元定理}:$\dim V = \dim \mathrm{Ker}\,f + \dim \mathrm{Im}\,f$。
証明は「核の基底を全体の基底に拡張し、付け足した分の行き先が像の基底になる」。
自由度定理はその行列版である。
\item 全単射な線形写像が\textbf{同型写像}。$n$ 次元空間はすべて
$\R^n$ と同型であり、座標写像がその同型を与える。
\end{itemize}
\end{matome}

\section*{章末問題}

\begin{mondai}[基本]\label{q:linmap-ex1}
次の写像はそれぞれ線形写像か。線形であるものは証明し、
そうでないものは理由を一つ挙げよ。\\
　(1) $f : \R^2 \to \R^2$, $f(x, y) = (x - y,\ 2x)$\\
　(2) $g : \R^2 \to \R^2$, $g(x, y) = (x + 2,\ y)$\\
　(3) $h : \R^2 \to \R^2$, $h(x, y) = (x^2,\ y)$
\end{mondai}

\begin{mondai}[基本]\label{q:linmap-ex2}
$A = \mat{1 & 2 \\ 3 & 6}$ の定める線形写像 $f : \R^2 \to \R^2$ の
核と像を求め(それぞれ基底を示せ)、次元定理を確かめよ。
\end{mondai}

\begin{mondai}[基本]\label{q:linmap-ex3}
線形写像 $f : \R^2 \to \R^2$ が
$f\mat{1 \\ 0} = \mat{2 \\ 1}$, $f\mat{0 \\ 1} = \mat{-1 \\ 1}$ を
満たすとき、$f\mat{3 \\ 2}$ を求めよ。また一般の $f\mat{x \\ y}$ を
成分で書き、$f$ を定める行列を答えよ。
\end{mondai}

\begin{mondai}[基本]\label{q:linmap-ex4}
微分 $D : \R[x]_2 \to \R[x]_2$ について、$\mathrm{Ker}\,D$ と
$\mathrm{Im}\,D$ を求め(それぞれ基底を示せ)、次元定理を確かめよ。
\end{mondai}

\begin{mondai}[標準]\label{q:linmap-ex5}
$T : \R[x]_2 \to \R$, $T(p) = \displaystyle\int_0^1 p(x)\,dx$ について、\\
　(1) $T$ が線形写像であることを示せ。\\
　(2) $\dim \mathrm{Ker}\,T$ を次元定理を用いて求めよ。\\
　(3) $\mathrm{Ker}\,T$ の基底を一組求めよ。
\end{mondai}

\begin{mondai}[標準]\label{q:linmap-ex6}
$T : \R[x]_2 \to \R^2$, $T(p) = \bigl(p(0),\ p(1)\bigr)$ について、
$\mathrm{Ker}\,T$ の基底と次元を求め、$T$ が全射かどうか、
単射かどうかを判定せよ。
\end{mondai}

\begin{mondai}[標準]\label{q:linmap-ex7}
トレース $\mathrm{tr} : M_{2,2} \to \R$(例題 \ref{rei:sub-trace})に
ついて、$\mathrm{Im}\,\mathrm{tr}$ と $\dim \mathrm{Ker}\,\mathrm{tr}$ を
求め、次元定理を確かめよ。
\end{mondai}

\begin{mondai}[標準]\label{q:linmap-ex8}
数列空間上のシフト写像
$S : (a_0, a_1, a_2, \dots) \mapsto (a_1, a_2, a_3, \dots)$ について、\\
　(1) $\mathrm{Ker}\,S$ を求めよ(どんな数列の集合か)。\\
　(2) $S$ は全射であること、しかし単射でないことを示せ。
(有限次元では起こらない現象である。問 \ref{q:linmap-ex11} と比較せよ。)
\end{mondai}

\begin{mondai}[発展]\label{q:linmap-ex9}
$2$ 回微分 $D^2 : \R[x]_3 \to \R[x]_3$($p \mapsto p''$)について、
$\mathrm{Ker}\,D^2$ と $\mathrm{Im}\,D^2$ の基底と次元を求め、
次元定理を確かめよ。
\end{mondai}

\begin{mondai}[発展]\label{q:linmap-ex10}
差分写像 $\Delta : \R[x]_2 \to \R[x]_2$,
$(\Delta p)(x) = p(x + 1) - p(x)$ について、\\
　(1) $\Delta$ が線形写像であることを示せ。\\
　(2) $\Delta(1)$, $\Delta(x)$, $\Delta(x^2)$ を計算せよ。\\
　(3) $\mathrm{Ker}\,\Delta$ と $\mathrm{Im}\,\Delta$ の基底と次元を求め、
次元定理を確かめよ。
\end{mondai}

\begin{mondai}[証明]\label{q:linmap-ex11}
$V, W$ を同じ次元 $n$ の有限次元ベクトル空間、$f : V \to W$ を
線形写像とする。このとき
「$f$ が単射 $\iff$ $f$ が全射」
を次元定理を用いて証明せよ。
\end{mondai}

\begin{mondai}[証明]\label{q:linmap-ex12}
線形写像 $f : V \to W$、$g : W \to U$ の合成 $g \circ f$ について、
次を証明せよ。\\
　(1) $\mathrm{Ker}\,f \subseteq \mathrm{Ker}\,(g \circ f)$\\
　(2) $\mathrm{Im}\,(g \circ f) \subseteq \mathrm{Im}\,g$\\
　(3) $V, W, U$ が有限次元のとき
$\dim \mathrm{Im}\,(g \circ f) \le \min\{\dim \mathrm{Im}\,f,\ \dim \mathrm{Im}\,g\}$
(行列でいえば $\mathrm{rank}\,(BA) \le \min\{\mathrm{rank}\,A,
\mathrm{rank}\,B\}$ にあたる)。
\end{mondai}
...
巻末「問の解答」にある。
\begin{itemize}
\item \textbf{問 \ref{q:linmap-ex1}}:まず $f(\vect{0})$ を見る。
$\vect{0}$ にうつるものは、和とスカラー倍の保存を文字で確かめる。
\item \textbf{問 \ref{q:linmap-ex2}}:核は同次方程式を解く。
像は列ベクトルの張る空間。$2$ 列が平行であることに注意。
\item \textbf{問 \ref{q:linmap-ex3}}:$\mat{3 \\ 2} = 3\vect{e}_1 + 2\vect{e}_2$
と分解して線形性を使う(例題 \ref{rei:linmap-fromdata})。
\item \textbf{問 \ref{q:linmap-ex4}}:微分して $0$ になる多項式、
微分の結果として現れる多項式をそれぞれ考える。
\item \textbf{問 \ref{q:linmap-ex5}}:(2) は $\mathrm{Im}\,T = \R$ を
先に示す($T(1)$ を計算)。(3) は $\int_0^1 p = 0$ を係数の条件に直す。
\item \textbf{問 \ref{q:linmap-ex6}}:$p(0) = p(1) = 0$ は
因数定理で「$x(x - 1)$ で割り切れる」と読める。
\item \textbf{問 \ref{q:linmap-ex7}}:$\mathrm{tr}\,E = 2 \neq 0$ から
像が $\R$ 全体と分かる。核は例題 \ref{rei:sub-trace} の空間である。
\item \textbf{問 \ref{q:linmap-ex8}}:(1) は $S$ でうつした結果が
零数列になる条件を書く。(2) 全射は「任意の数列の先頭に $0$ を足した
数列」を考える。単射でないことは核から。
\item \textbf{問 \ref{q:linmap-ex9}}:$2$ 回微分して消えるのは
$1$ 次以下。像は何次以下になるか。
\item \textbf{問 \ref{q:linmap-ex10}}:(2) の計算結果を並べれば
像の生成系が得られる。核は $\Delta p = 0$ を係数比較で解く。
\item \textbf{問 \ref{q:linmap-ex11}}:単射 $\iff \dim \mathrm{Ker} = 0$、
全射 $\iff \dim \mathrm{Im} = n$(定理 \ref{thm:dim-sub}(2) を使う)。
両者を次元定理でつなぐ。
\item \textbf{問 \ref{q:linmap-ex12}}:(1)(2) は定義どおり元をとって追う。
(3) は (2) と、「$g$ を $\mathrm{Im}\,f$ に制限した写像に次元定理を使う」
または $\mathrm{Im}(g \circ f) = g(\mathrm{Im}\,f)$ に注目する。
\end{itemize}
}
...
\begin{goal}
\begin{itemize}
\item 基底を選ぶことで、抽象的な線形写像を\textbf{表現行列}という
具体的な行列に翻訳できるようになる(「第 $j$ 列は
$f(\vect{e}_j)$ の座標」)。
\item 基底を取り替えたとき、座標が $\vect{c} = P\vect{c}'$、表現行列が
$A' = P^{-1}AP$ と変わることを、導出込みで理解する。
\item \textbf{相似}な行列は「同じ変換の別の座標表示」であることを理解し、
行列式・トレースなどの\textbf{相似不変量}を、相似でないことの判定に
使えるようになる。
\end{itemize}
\end{goal}

\section{動機 ― 抽象と計算をつなぐ辞書}

第III部で私たちは、多項式や数列までベクトルとみなし、微分や差分まで
線形写像とみなす抽象の高みに登った。だが計算の道具は、あいかわらず
第I・II部の行列と掃き出し法である。この二つの世界をつなぐ辞書が本章の
主題、\textbf{表現行列}である。

辞書の原理はすでに揃っている。第 \ref{chap:basis} 章で、基底を選べば
どんな $n$ 次元空間のベクトルも座標 $(c_1, \dots, c_n)$ で表せることを見た
(同型 $V \cong \R^n$)。第 \ref{chap:linmap} 章で、線形写像は基底の
行き先だけで決まることを見た(定理 \ref{thm:linmap-basis})。
ならば、\textbf{基底の行き先を座標で書いて並べれば}、写像の全情報が
$1$ 枚の行列に収まるはずである。

ただし、翻訳には代償が伴う。基底の選び方は自由だから、
\textbf{同じ写像が基底ごとに違う行列で表される}。
この「翻訳のゆらぎ」を制御する法則が基底変換の公式 $A' = P^{-1}AP$ であり、
ゆらぎに影響されない量(不変量)を探す視点が、第IV部の固有値理論へと
つながっていく。
...
\begin{teiri}[表現行列は座標の掛け算になる]\label{thm:repmat-coord}
$\vect{v} \in V$ の座標を $\vect{c} = (c_1, \dots, c_n)$ とすると、
$f(\vect{v})$ の座標は $A\vect{c}$ である。
\end{teiri}

\noindent\textbf{(証明)}\quad
$\vect{v} = c_1\vect{e}_1 + \cdots + c_n\vect{e}_n$ に $f$ を施すと、
線形性(定理 \ref{thm:linmap-basic}(3))より
\[
f(\vect{v}) = \sum_{j=1}^n c_j\,f(\vect{e}_j)
= \sum_{j=1}^n c_j \sum_{i=1}^n a_{ij}\vect{e}_i
= \sum_{i=1}^n \Bigl( \sum_{j=1}^n a_{ij}c_j \Bigr) \vect{e}_i.
\]
$\vect{e}_i$ の係数 $\sum_j a_{ij}c_j$ は、ちょうど $A\vect{c}$ の
第 $i$ 成分である。 $\blacksquare$

抽象的な写像の計算が、行列の掛け算に完全に翻訳される。
$V = \R^n$ で標準基底をとれば、表現行列は第I部の
「線形変換を定める行列」(定理 \ref{thm:lin-is-matrix})そのものである。
...
\begin{teiri}[合成の表現行列は行列の積]\label{thm:repmat-comp}
同じ基底に関する $f$ の表現行列を $A$、$g$ の表現行列を $B$ とすると、
合成 $g \circ f$ の表現行列は積 $BA$ である。
\end{teiri}

\noindent\textbf{(証明)}\quad
$\vect{v}$ の座標を $\vect{c}$ とすると、定理 \ref{thm:repmat-coord} より
$f(\vect{v})$ の座標は $A\vect{c}$、さらに $g$ を施した
$g(f(\vect{v}))$ の座標は $B(A\vect{c}) = (BA)\vect{c}$ である。
座標の一意性より、$g \circ f$ の表現行列は $BA$ である。 $\blacksquare$

第 \ref{chap:matmul} 章で「行列の積は写像の合成を表すように定義した」
ことを思い出そう。あの定義が、多項式や数列の空間の上の写像にまで、
そのまま通用するのである。たとえば「$2$ 回微分」の表現行列は $A^2$ で
計算できる(章末問題 \ref{q:repmat-ex9})。
...
\begin{teigi}[基底変換行列]\label{def:changebasis}
$V$ の旧基底 $\vect{e}_1, \dots, \vect{e}_n$ と
新基底 $\vect{e}'_1, \dots, \vect{e}'_n$ に対し、各新基底ベクトルを
旧基底で展開して
\[
\vect{e}'_j = p_{1j}\vect{e}_1 + p_{2j}\vect{e}_2 + \cdots + p_{nj}\vect{e}_n
\]
と書いたときの係数を並べた行列 $P = (p_{ij})$ を\textbf{基底変換行列}という
(\textbf{第 $j$ 列には $\vect{e}'_j$ の旧座標が入る})。
$V = \R^n$ で旧基底が標準基底なら、$P$ は新基底のベクトルを
そのまま列に並べた行列である。
\end{teigi}

\begin{teiri}[座標の変換則]\label{thm:coordchange}
基底変換行列 $P$ は正則であり、ベクトル $\vect{v}$ の
旧座標 $\vect{c}$ と新座標 $\vect{c}'$ は
\[
\vect{c} = P\vect{c}', \qquad \vect{c}' = P^{-1}\vect{c}
\]
で結ばれる。
\end{teiri}

\noindent\textbf{(証明)}\quad
$\vect{v} = \sum_j c'_j \vect{e}'_j$ に $\vect{e}'_j$ の展開を代入すると
\[
\vect{v} = \sum_j c'_j \sum_i p_{ij}\vect{e}_i
= \sum_i \Bigl( \sum_j p_{ij}c'_j \Bigr) \vect{e}_i
\]
となり、旧座標の一意性から $c_i = \sum_j p_{ij}c'_j$、すなわち
$\vect{c} = P\vect{c}'$ である。
正則性:逆向きに、旧基底を新基底で展開した行列を $Q$ とすると、
同じ議論で $\vect{c}' = Q\vect{c}$。合わせると
$\vect{c} = PQ\vect{c}$ がすべての $\vect{c} \in \R^n$ で成り立つから
$PQ = E$。よって $P$ は正則で($Q = P^{-1}$)、
$\vect{c}' = P^{-1}\vect{c}$ である。 $\blacksquare$

\begin{chui}[向きに注意]
$P$ の列に入っているのは\textbf{新}基底の旧座標なのに、
座標の変換は $\vect{c} = P\vect{c}'$(新座標に $P$ を掛けると
\textbf{旧}座標に戻る)と、向きが逆に見える。
「$P$ は新しい住所を古い住所に翻訳する行列」と覚えるとよい。
新座標がほしければ $P^{-1}$ を掛ける。
\end{chui}

\begin{teiri}[表現行列の変換則]\label{thm:repmat-change}
線形写像 $f : V \to V$ の旧基底での表現行列を $A$、
新基底での表現行列を $A'$ とすると
\[
A' = P^{-1} A P.
\]
...
\noindent\textbf{(証明)}\quad
$\vect{v}$ の新座標を $\vect{c}'$ とする。$f(\vect{v})$ の新座標を
二通りの経路で計算する。まず旧座標に翻訳すると $\vect{c} = P\vect{c}'$
(定理 \ref{thm:coordchange})。旧基底での表現行列 $A$ により、
$f(\vect{v})$ の旧座標は $AP\vect{c}'$(定理 \ref{thm:repmat-coord})。
これを新座標に翻訳し直すと $P^{-1}AP\vect{c}'$ である。
一方、新基底での表現行列 $A'$ の定義から、$f(\vect{v})$ の新座標は
$A'\vect{c}'$ でもある。すべての $\vect{c}'$ で
$A'\vect{c}' = P^{-1}AP\vect{c}'$ が成り立つから、
$A' = P^{-1}AP$ である。 $\blacksquare$

「新座標 $\to$($P$ で)旧座標 $\to$($A$ で)写像を実行 $\to$
($P^{-1}$ で)新座標に戻る」という三段構えが、式 $P^{-1}AP$ の
右から左への読み方である。

\begin{teigi}[相似]
正則行列 $P$ により $B = P^{-1}AP$ と書けるとき、
$A$ と $B$ は\textbf{相似}であるという。
\end{teigi}
...
\section{相似不変量 ― 座標によらない量}

相似な行列は同じ変換の別表示だから、\textbf{相似で変わらない量}こそが
変換そのものの性質を表す。代表が行列式とトレースである。
行列式については、積の公式(定理 \ref{thm:det-prod})から直ちに
\[
\det(P^{-1}AP) = \det P^{-1} \det A \det P
= \frac{1}{\det P}\,\det A\,\det P = \det A
\]
と分かる。トレース(対角成分の和
$\mathrm{tr}\,A = a_{11} + \cdots + a_{nn}$)についても証明しよう。

\begin{teiri}[トレースの相似不変性]\label{thm:trace-similar}
\begin{enumerate}
\item 任意の $n$ 次正方行列 $A, B$ について
...
\noindent\textbf{(証明)}\quad
(1) 積の定義から $(AB)_{ii} = \sum_k a_{ik}b_{ki}$ なので
\[
\mathrm{tr}(AB) = \sum_{i=1}^n \sum_{k=1}^n a_{ik}b_{ki}, \qquad
\mathrm{tr}(BA) = \sum_{k=1}^n \sum_{i=1}^n b_{ki}a_{ik}.
\]
どちらも「すべての添字の組 $(i, k)$ にわたる $a_{ik}b_{ki}$ の総和」で、
足す順序が違うだけだから等しい。

(2) (1) で $A$ の代わりに $P^{-1}A$、$B$ の代わりに $P$ をとると
\[
\mathrm{tr}(P^{-1}AP) = \mathrm{tr}\bigl((P^{-1}A)P\bigr)
= \mathrm{tr}\bigl(P(P^{-1}A)\bigr) = \mathrm{tr}\,A. \qquad\blacksquare
\]

$AB \neq BA$ なのに $\mathrm{tr}(AB) = \mathrm{tr}(BA)$ は必ず成り立つ
――トレースは行列の非可換性を「ならして」しまう不思議な量である。
行列式・トレース・階数はどれも相似不変であり、しかも第IV部で学ぶ
\textbf{固有値}から作られる量であることが分かってくる
(行列式は固有値の積、トレースは固有値の和)。
...
行列式(拡大率)とトレースの相似不変性は本文で示した。
階数も、そして第IV部で学ぶ\textbf{固有値}も相似不変である。
...
問 \ref{q:repmat} は、その最初の実演である。
\end{itembox}

\section{反例 ― ありがちな誤解を正す}

\begin{hanrei}[表現行列は基底の順序にも依存する]\label{hanrei:repmat-order}
「基底が同じ集合なら表現行列も同じ」ではない。
\textbf{基底は順序込み}で指定するものである。
微分 $D : \R[x]_2 \to \R[x]_2$ を、順序を逆にした基底
$x^2,\ x,\ 1$ で表してみると、
$D(x^2) = 2x$(座標 $(0, 2, 0)$)、$D(x) = 1$(座標 $(0, 0, 1)$)、
$D(1) = 0$ より
\[
A' = \mat{0 & 0 & 0 \\ 2 & 0 & 0 \\ 0 & 1 & 0}
\]
となり、基底 $1, x, x^2$ での表現行列(上三角)とは別物(下三角)になる。
もちろん両者は相似で、トレース($0$)も行列式($0$)も一致する。
\end{hanrei}

\begin{hanrei}[不変量が一致しても相似とは限らない]\label{hanrei:repmat-invariant}
相似なら行列式・トレースは一致するが、\textbf{逆は成り立たない}。
\[
A = \mat{2 & 0 \\ 0 & 2} = 2E, \qquad B = \mat{2 & 1 \\ 0 & 2}
\]
は $\mathrm{tr} = 4$、$\det = 4$ と両不変量が一致する。
しかし相似ではない:どんな正則行列 $P$ に対しても
\[
P^{-1}(2E)P = 2\,P^{-1}EP = 2E \neq B
\]
だから、$2E$ と相似な行列は $2E$ 自身しかない。
不変量は「相似で\textbf{ない}ことの証明」には使えるが
(値が違えば即座に非相似)、「相似で\textbf{ある}ことの証明」には
一般に不十分である。相似の完全な判定は第VI部のジョルダン標準形で与えられる。
\end{hanrei}

\section{例題}

表現行列の作成・利用と、基底変換の計算を練習する。
以下は\textbf{解答つきの例題}である。手を動かして追ってほしい。

\begin{rei}[例題・基本(標準基底に関する表現行列)]\label{rei:repmat-std}
線形変換 $f : \R^2 \to \R^2$, $f(x, y) = (x + 2y,\ 3y)$ の
標準基底に関する表現行列を求めよ。\\
\textbf{解.}\ 基底ベクトルの行き先を計算して列に並べる。
\[
f\mat{1 \\ 0} = \mat{1 \\ 0}, \qquad f\mat{0 \\ 1} = \mat{2 \\ 3}
\]
より、表現行列は $A = \mat{1 & 2 \\ 0 & 3}$ である。
検算:$A\mat{x \\ y} = \mat{x + 2y \\ 3y}$ で定義と一致する。
\end{rei}

\begin{rei}[例題・基本(うまい基底なら対角になる)]\label{rei:repmat-mirror}
鏡映 $f : \R^2 \to \R^2$, $f(x, y) = (y, x)$(直線 $y = x$ に関する
折り返し)の表現行列を、
(i) 標準基底、(ii) 基底 $\vect{e}'_1 = \mat{1 \\ 1}$,
$\vect{e}'_2 = \mat{1 \\ -1}$ のそれぞれについて求めよ。\\
\textbf{解.}\ (i) $f(\vect{e}_1) = \mat{0 \\ 1}$,
$f(\vect{e}_2) = \mat{1 \\ 0}$ を列に並べて
$A = \mat{0 & 1 \\ 1 & 0}$。

(ii) 行き先を\textbf{新基底で}展開する。
$f(\vect{e}'_1) = \mat{1 \\ 1} = 1 \cdot \vect{e}'_1 + 0 \cdot \vect{e}'_2$、
$f(\vect{e}'_2) = \mat{-1 \\ 1} = 0 \cdot \vect{e}'_1 + (-1)\vect{e}'_2$
より
\[
A' = \mat{1 & 0 \\ 0 & -1}.
\]
鏡映の「本性」――鏡の向き($y = x$ 方向)は不動、垂直方向は反転――が、
基底をうまく選ぶと対角行列としてむき出しになる。
$\mathrm{tr}$ は $0$、$\det$ は $-1$ で、(i) と一致していることも確認できる。
\end{rei}

\begin{rei}[例題・基本(座標の変換)]\label{rei:repmat-coordchg}
$\R^2$ の新基底 $\vect{e}'_1 = \mat{1 \\ 1}$,
$\vect{e}'_2 = \mat{1 \\ -1}$ について基底変換行列 $P$ を書き、
$\vect{v} = \mat{4 \\ 2}$ の新座標を求めよ。\\
\textbf{解.}\ 旧基底は標準基底だから、$P$ は新基底を列に並べて
$P = \mat{1 & 1 \\ 1 & -1}$。新座標は
$\vect{c}' = P^{-1}\vect{v}$(定理 \ref{thm:coordchange})である。
$\det P = -2$ より
$P^{-1} = \dfrac{1}{-2}\mat{-1 & -1 \\ -1 & 1}
= \dfrac{1}{2}\mat{1 & 1 \\ 1 & -1}$、
\[
\vect{c}' = \frac{1}{2}\mat{1 & 1 \\ 1 & -1}\mat{4 \\ 2}
= \frac{1}{2}\mat{6 \\ 2} = \mat{3 \\ 1}.
\]
検算:$3\mat{1 \\ 1} + 1\mat{1 \\ -1} = \mat{4 \\ 2}$。
\end{rei}

\begin{rei}[例題・標準(変換則の実演)]\label{rei:repmat-change-ex}
標準基底に関する表現行列が $A = \mat{2 & 1 \\ 1 & 2}$ の線形変換 $f$ を、
基底 $\vect{e}'_1 = \mat{1 \\ 1}$, $\vect{e}'_2 = \mat{1 \\ -1}$ で
表せ($P^{-1}AP$ の計算と、行き先の直接展開の両方で求めて一致を確かめよ)。\\
\textbf{解.}\ \textbf{方法1($P^{-1}AP$)}:
$P = \mat{1 & 1 \\ 1 & -1}$,
$P^{-1} = \dfrac{1}{2}\mat{1 & 1 \\ 1 & -1}$(例題 \ref{rei:repmat-coordchg})。
\[
AP = \mat{2 & 1 \\ 1 & 2}\mat{1 & 1 \\ 1 & -1} = \mat{3 & 1 \\ 3 & -1},
\qquad
A' = P^{-1}(AP) = \frac{1}{2}\mat{1 & 1 \\ 1 & -1}\mat{3 & 1 \\ 3 & -1}
= \mat{3 & 0 \\ 0 & 1}.
\]
\textbf{方法2(直接展開)}:
$A\vect{e}'_1 = \mat{3 \\ 3} = 3\vect{e}'_1$、
$A\vect{e}'_2 = \mat{1 \\ -1} = 1 \cdot \vect{e}'_2$ だから、
係数を列に並べて $A' = \mat{3 & 0 \\ 0 & 1}$。両方法が一致した。
$\vect{e}'_1$ 方向に $3$ 倍、$\vect{e}'_2$ 方向に $1$ 倍
――この基底は $f$ の「軸」を捉えている(第IV部の固有ベクトルである)。
\end{rei}

\begin{rei}[例題・標準(多項式空間での基底取り替え)]\label{rei:repmat-poly}
微分 $D : \R[x]_2 \to \R[x]_2$ の、基底
$1,\ 1 + x,\ (1 + x)^2$(問 \ref{q:basis1}(2))に関する表現行列を求めよ。\\
\textbf{解.}\ 各基底ベクトルの行き先を、\textbf{同じ基底で}展開する。
\[
D(1) = 0, \qquad
D(1 + x) = 1, \qquad
D\bigl((1 + x)^2\bigr) = 2(1 + x).
\]
座標はそれぞれ $(0,0,0)$, $(1,0,0)$, $(0,2,0)$ だから
\[
A' = \mat{0 & 1 & 0 \\ 0 & 0 & 2 \\ 0 & 0 & 0}.
\]
これは基底 $1, x, x^2$ での表現行列とまったく同じである。
$1 + x$ は「$x$ を平行移動した変数」であり、微分は平行移動と交換する
($p(x + 1)$ の微分は $p'(x + 1)$)ため、行列が変わらないのである。
\textbf{基底を替えても行列が変わらないことも、変わることもある}
――どう変わるかを統制するのが変換則 $A' = P^{-1}AP$ である。
\end{rei}

\begin{rei}[例題・標準(相似でないことの証明)]\label{rei:repmat-notsim}
次の各組の行列が相似で\textbf{ない}ことを示せ。
\[
(1)\ \mat{1 & 2 \\ 2 & 1} \text{ と } \mat{1 & 1 \\ 1 & 1} \qquad
(2)\ \mat{0 & 1 \\ -1 & 0} \text{ と } \mat{1 & 0 \\ 0 & -1}
\]
\textbf{解.}\ 相似なら行列式もトレースも一致するはずである(対偶を使う)。\\
　(1) 行列式は $1 - 4 = -3$ と $1 - 1 = 0$ で異なる。よって相似でない。\\
　(2) 行列式はどちらも $1$ と $-1$ ……ではなく、
左は $0 - (-1) = 1$、右は $-1$ で異なる。よって相似でない
(トレースはどちらも $0$ で一致するので、トレースだけでは判定できない
ことにも注意)。
幾何的にいえば、左は $90^\circ$ 回転(向きを保つ、$\det = 1$)、
右は鏡映(向きを裏返す、$\det = -1$)であり、
座標の取り替えでは互いにうつり合えない変換である。
\end{rei}

\begin{matome}
\begin{itemize}
\item \textbf{表現行列}:基底を選び、$f(\vect{e}_j)$ の座標を第 $j$ 列に
並べた行列。$\vect{v}$ の座標が $\vect{c}$ なら $f(\vect{v})$ の座標は
$A\vect{c}$(定理 \ref{thm:repmat-coord})。合成は積 $BA$ に対応する
(定理 \ref{thm:repmat-comp})。
\item \textbf{基底変換行列} $P$:新基底の旧座標を列に並べた行列(正則)。
座標は $\vect{c} = P\vect{c}'$、$\vect{c}' = P^{-1}\vect{c}$ で移り合う
(「$P$ は新住所を旧住所に翻訳する」)。
\item \textbf{表現行列の変換則}:$A' = P^{-1}AP$。この関係にある行列を
\textbf{相似}といい、相似な行列は同じ変換の別の座標表示である。
\item 行列式・トレースは\textbf{相似不変量}
($\mathrm{tr}(AB) = \mathrm{tr}(BA)$ から従う。
定理 \ref{thm:trace-similar})。不変量が違えば相似でないと判定できるが、
一致しても相似とは限らない(反例 \ref{hanrei:repmat-invariant})。
\item うまい基底を選ぶと表現行列は劇的に簡単になる(対角になる)ことがある。
その体系的な方法=対角化が第IV部の主題である。
\end{itemize}
\end{matome}

\section*{章末問題}

\begin{mondai}[基本]\label{q:repmat-ex1}
線形変換 $f : \R^2 \to \R^2$, $f(x, y) = (2x - y,\ x + 3y)$ の
標準基底に関する表現行列を求めよ。
\end{mondai}

\begin{mondai}[基本]\label{q:repmat-ex2}
微分 $D : \R[x]_3 \to \R[x]_3$ の基底 $1, x, x^2, x^3$ に関する
表現行列 $A$ を求めよ。また $p(x) = 1 + x + x^2 + x^3$ の座標に
$A$ を掛けることで $p'(x)$ を計算し、直接微分した結果と一致することを
確かめよ。
\end{mondai}

\begin{mondai}[基本]\label{q:repmat-ex3}
$\R^2$ の基底 $\vect{e}'_1 = \mat{2 \\ 1}$, $\vect{e}'_2 = \mat{1 \\ 1}$
について基底変換行列 $P$ と $P^{-1}$ を求め、
$\vect{v} = \mat{1 \\ 4}$ の新座標を計算せよ
(問 \ref{q:dim-ex2} の結果と一致するはずである)。
\end{mondai}

\begin{mondai}[基本]\label{q:repmat-ex4}
恒等写像 $\mathrm{id} : V \to V$($\vect{v} \mapsto \vect{v}$)の
表現行列は、どの基底に関しても単位行列 $E$ であることを示せ。
また、スカラー倍写像 $\vect{v} \mapsto 3\vect{v}$ についてはどうか。
\end{mondai}

\begin{mondai}[標準]\label{q:repmat-ex5}
標準基底に関する表現行列が $A = \mat{1 & 2 \\ 0 & 3}$ の線形変換を、
基底 $\vect{e}'_1 = \mat{1 \\ 0}$, $\vect{e}'_2 = \mat{1 \\ 1}$ で表した
表現行列 $A'$ を、
(i) $P^{-1}AP$ の計算、(ii) $A\vect{e}'_j$ を新基底で展開する方法、
の両方で求め、一致することを確かめよ。
\end{mondai}

\begin{mondai}[標準]\label{q:repmat-ex6}
微分 $D : \R[x]_2 \to \R[x]_2$ の、基底
$1,\ x,\ \dfrac{x^2}{2}$ に関する表現行列を求めよ。
\end{mondai}

\begin{mondai}[標準]\label{q:repmat-ex7}
写像 $f : \R[x]_1 \to \R[x]_1$ を $f(p)(x) = p(2x + 1)$ で定める
($p$ に $2x + 1$ を代入した多項式を対応させる)。\\
　(1) $f$ が線形写像であることを示せ。\\
　(2) 基底 $1, x$ に関する表現行列 $A$ を求めよ。\\
　(3) $p(x) = 3 + 4x$ に対し、座標に $A$ を掛けて $f(p)$ を求め、
直接代入した結果と一致することを確かめよ。
\end{mondai}

\begin{mondai}[標準]\label{q:repmat-ex8}
次の各組の行列が相似でないことを、相似不変量を用いて示せ。
\[
(1)\ \mat{1 & 1 \\ 0 & 1} \text{ と } \mat{1 & 0 \\ 2 & 2} \qquad
(2)\ \mat{2 & 0 \\ 0 & 3} \text{ と } \mat{2 & 0 \\ 0 & -3}
\]
\end{mondai}

\begin{mondai}[発展]\label{q:repmat-ex9}
微分 $D : \R[x]_2 \to \R[x]_2$ の基底 $1, x, x^2$ に関する表現行列を
$A$ とする。$A^2$ と $A^3$ を計算し、それぞれが
「$2$ 回微分」「$3$ 回微分」の表現行列になっていること
(定理 \ref{thm:repmat-comp})を、$D^2(x^2)$, $D^3(x^2)$ などの
直接計算で確かめよ。
\end{mondai}

\begin{mondai}[発展]\label{q:repmat-ex10}
$90^\circ$ 回転の標準基底に関する表現行列は
$A = \mat{0 & -1 \\ 1 & 0}$ である。基底
$\vect{e}'_1 = \mat{1 \\ 1}$, $\vect{e}'_2 = \mat{1 \\ -1}$ に関する
表現行列 $A' = P^{-1}AP$ を計算せよ。$A'$ は対角行列になるか。
(実は、どんな実基底を選んでも回転の表現行列は対角にならない。
その理由は第 \ref{chap:diag} 章の深掘りで明かされる。)
\end{mondai}

\begin{mondai}[証明]\label{q:repmat-ex11}
線形写像 $f : V \to V$ の、ある基底に関する表現行列を $A$ とする。
次を証明せよ。\\
　(1) $f$ が同型写像ならば $A$ は正則であり、$f^{-1}$ の表現行列は
$A^{-1}$ である。\\
　(2) 逆に $A$ が正則ならば $f$ は同型写像である。
(ヒント:定理 \ref{thm:repmat-coord} と、座標写像が同型であること
(定理 \ref{thm:iso-rn})を使う。)
\end{mondai}

\begin{mondai}[証明]\label{q:repmat-ex12}
行列の相似は\textbf{同値関係}であること、すなわち次を証明せよ。\\
　(1) $A$ は $A$ 自身と相似(反射律)。\\
　(2) $A$ が $B$ と相似ならば、$B$ は $A$ と相似(対称律)。\\
　(3) $A$ が $B$ と相似、$B$ が $C$ と相似ならば、$A$ は $C$ と相似
(推移律)。
\end{mondai}
...
巻末「問の解答」にある。
\begin{itemize}
\item \textbf{問 \ref{q:repmat-ex1}}:$f(\vect{e}_1)$, $f(\vect{e}_2)$ を
計算して列に並べる。
\item \textbf{問 \ref{q:repmat-ex2}}:$D(x^k) = kx^{k-1}$ の座標を
第 $k+1$ 列に入れる。$4 \times 4$ になる。
\item \textbf{問 \ref{q:repmat-ex3}}:$P$ は新基底を列に並べる。
$2 \times 2$ の逆行列公式で $P^{-1}$。新座標は $P^{-1}\vect{v}$。
\item \textbf{問 \ref{q:repmat-ex4}}:$\mathrm{id}(\vect{e}_j) = \vect{e}_j$
の座標はどうなるかを考える。
\item \textbf{問 \ref{q:repmat-ex5}}:方法 (ii) では
$A\vect{e}'_j = c_1\vect{e}'_1 + c_2\vect{e}'_2$ の $c_1, c_2$ を
連立方程式で求める。
\item \textbf{問 \ref{q:repmat-ex6}}:$D(x^2/2) = x$ に注意。
できあがる行列は例題 \ref{rei:repmat-poly} のものと見比べよ。
\item \textbf{問 \ref{q:repmat-ex7}}:$f(1)$ と $f(x)$ を計算して
列に並べる。$1$ の行き先も忘れずに。
\item \textbf{問 \ref{q:repmat-ex8}}:トレースと行列式を計算して比べる。
どちらか一方でも違えば相似でない。
\item \textbf{問 \ref{q:repmat-ex9}}:行列の積を計算し、
「$k$ 回微分の表現行列」と読み比べる。$A^3 = O$ になる。
\item \textbf{問 \ref{q:repmat-ex10}}:例題 \ref{rei:repmat-change-ex} と
同じ手順。結果のトレースと行列式が $A$ と一致することも確かめよ。
\item \textbf{問 \ref{q:repmat-ex11}}:座標写像 $\varphi$ を使うと
$f$ と「$A$ を掛ける写像」が $\varphi$ を介して対応する。
合成 $f \circ f^{-1} = \mathrm{id}$ の表現行列を考える。
\item \textbf{問 \ref{q:repmat-ex12}}:(1) は $P = E$。(2) は
$Q = P^{-1}$ とおく。(3) は二つの変換行列の積を作る。
\end{itemize}
}
